/**
 * Drag & Drop Result PDF — server functions.
 *
 * Two endpoints for therapist use:
 *   - generateDragDropResultPatient  → same PDF the patient gets
 *   - generateDragDropResultTherapist → clinical version with metrics + flags
 *
 * Magic link auto-generation uses buildDragDropResultPDF directly
 * (called inside submitActivityResponse, no auth session).
 */
...
import { buildDragDropResultPDF } from "./drag-drop-result-pdf.server";
...
export const generateDragDropResultPatient = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const pdfBuffer = await buildDragDropResultPDF({
    ...
    await recordAudit({
      actorId: userId,
      workspaceId: data.workspaceId,
      action: "drag_drop_result.patient_generated",
...
export const generateDragDropResultTherapist = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => ResultSchema.parse(input))
  .handler(async ({ data, context }) => {
    const { userId, supabase } = context;
    ...
    await recordAudit({
      actorId: userId,
      workspaceId: data.workspaceId,
      action: "drag_drop_result.therapist_generated",
      resourceType: "activity_response",
      resourceId: data.activityResponseId,
      metadata: {},
    });
